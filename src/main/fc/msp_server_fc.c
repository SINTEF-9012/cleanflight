...
#define BM(x) (1 << (x))
    const uint32_t rcModeCopyMask = BM(BOXHEADADJ) | BM(BOXCAMSTAB) | BM(BOXCAMTRIG) | BM(BOXBEEPERON)
        ...
        | BM(BOXTELEMETRY) | BM(BOXGTUNE) | BM(BOXBLACKBOX) | BM(BOXAIRMODE) | BM(BOXMOTORTEST) | BM(BOXRCOFFSETS) | BM(BOXVTX);
...
int mspServerCommandHandler(mspPacket_t *cmd, mspPacket_t *reply)
{
    ...
    switch (cmd->cmd) {
        ...
        // JAKOB SINTEF - New message type that can set offset RC values, but reset after say .5 seconds, to disable if sending code crashes
        case MSP_SET_OFFSET_RC: {
        	uint8_t channelCount = len / sizeof(int16_t);
        	if (channelCount > 4)
        		return -1;
        	int16_t frame[4];

        	for (unsigned i = 0; i < channelCount; i++) {
				frame[i] = sbufReadS16(src);
			}

        	rxMspOffsetFrameReceive(frame, channelCount);
			break;
		}
